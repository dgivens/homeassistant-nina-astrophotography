...
async def test_uninitialised_sequencer_is_no_data_not_an_error(status) -> None:
    ...
    """
    client = _client(
        FakeSession({"sequence/json": failure("Sequence is not initialized", status)})
    )
...
async def test_a_5xx_carrying_the_no_data_text_is_still_unavailable() -> None:
    ...
    client = _client(
        FakeSession({"sequence/json": failure("Sequence is not initialized", 500)})
    )
...
async def test_a_device_refusal_worded_not_initialized_is_a_command_error() -> None:
    ...
    """
    client = _client(
        FakeSession({"set-light": failure("Flat device is not initialized", 409)})
    )
...
async def test_a_command_error_carries_the_envelopes_code_and_message(
    attribute, expected
) -> None:
    client = _client(
...
async def test_a_pre_handler_html_status_is_classified_by_its_code(
    status, error
) -> None:
    ...
    session = FakeSession(
        {
            "flats/status": FakeResponse(
                f"<html>{status}</html>", status=status, content_type="text/html"
            )
        }
    )
...
@pytest.mark.parametrize(
    "exc", [aiohttp.ClientError("boom"), TimeoutError()], ids=["dropped", "timeout"]
)
...
async def test_get_versions_yields_both_version_strings() -> None:
    ...
    assert await _client(session).get_versions() == VersionInfo(
        "2.2.15.2", "3.2.0.9001"
    )
...
async def test_profile_is_mapped_from_the_active_profile() -> None:
    session = FakeSession(
        {"profile/show": ok({"TelescopeSettings": {"FocalLength": 500}})}
    )
...
async def test_frames_are_mapped_from_the_history_list() -> None:
    ...
    frames = await _client(FakeSession({"image-history": wire})).get_frames(
        include_all=True
    )
...
async def test_the_sequence_tree_is_mapped() -> None:
    client = _client(
        FakeSession({"sequence/json": load_envelope("dawn_sequence_complete.json")})
    )
...
async def test_the_cached_rig_offset_is_readable() -> None:
    ...
    client = _client(
        FakeSession({"equipment/info": load_envelope("dawn_equipment_info.json")})
    )
...
async def test_a_zero_offset_replaces_a_stale_one() -> None:
    ...
    session = FakeSession(
        {
            "equipment/info": clock("2026-09-04T08:11:22-05:00"),
            "event-history": ok(
...
async def test_an_unmappable_event_is_skipped_not_fatal() -> None:
    wire = ok(
        [
            {"Event": "GHOST"},
            {"Event": "SAFETY-CONNECTED", "Time": "2026-09-03T19:41:28-05:00"},
        ]
    )
...
@pytest.mark.parametrize(
    ...
    [
        # camera
        (
            lambda c: c.cool_camera(-10.0),
            "/equipment/camera/cool",
            {"temperature": -10.0, "minutes": -1},
        ),
        (lambda c: c.warm_camera(), "/equipment/camera/warm", {"minutes": -1}),
        (
            lambda c: c.set_target_temperature(-10.0),
            "/equipment/camera/cool",
            {"temperature": -10.0},
        ),
        (
            lambda c: c.set_target_temperature(-10.0, minutes=5),
            "/equipment/camera/cool",
        ...
        (
            lambda c: c.capture_image(5, gain=100, save=True),
            "/equipment/camera/capture",
        ...
        # guider
        (
            lambda c: c.start_guiding(),
            "/equipment/guider/start",
            {"calibrate": "false"},
        ),
        (
            lambda c: c.start_guiding(force_calibration=True),
            "/equipment/guider/start",
            {"calibrate": "true"},
        ),
        ...
        (
            lambda c: c.set_rotator_reverse(True),
            "/equipment/rotator/reverse",
        ...
        (
            lambda c: c.set_dome_follow(True),
            "/equipment/dome/set-follow",
            {"enabled": "true"},
        ),
        # flat device
        (
            lambda c: c.set_flat_light(True),
            "/equipment/flatdevice/set-light",
            {"on": "true"},
        ),
        (
            lambda c: c.set_flat_brightness(2048),
            "/equipment/flatdevice/set-brightness",
            {"brightness": 2048},
        ),
        (
            lambda c: c.open_flat_cover(),
            "/equipment/flatdevice/set-cover",
            {"closed": "false"},
        ),
        (
            lambda c: c.close_flat_cover(),
            "/equipment/flatdevice/set-cover",
            {"closed": "true"},
        ),
        # switch
        (
            lambda c: c.set_switch_value(2, 1.0),
...
async def test_the_livestack_route_quotes_its_two_path_segments() -> None:
    ...
    """
    session = FakeSession(
        {"/livestack/image/": FakeResponse(b"\xff\xd8", content_type="image/jpeg")}
    )
