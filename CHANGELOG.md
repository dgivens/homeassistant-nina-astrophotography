...
- **Ten sensors off the last autofocus report**, on the Focuser device: the
  ...
  `autofocus_hfr` is the lowest HFR the
  sweep MEASURED rather than `CalculatedFocusPoint.Value`, which under a
  `TREND*` fitting is the mean of the trendline intersection and the quadratic
  minimum and reads far below anything the camera saw; that value is the
...
- `sensor.<instance>_last_image_mean_adu` carries a `recent_frames` attribute
  ...
  tag. A strip click that fails to load no longer retitles the panel. Each
  thumbnail is a button a keyboard reaches and Enter or Space opens, named by
  its frame, type and filter.
...
- **The frame statistics card gives each filter the same colour every night.**
  ...
  `Ha 3nm`, `OIII`), with `Clear` and `UV/IR Cut` as L; any other filter takes
  a colour from its name alone, which two such filters may share. A night with
  no filter names — a one-shot-colour camera — draws each chart in its own
  colour; beside named filters, a light with no name is a hollow grey point on
  a dashed line.
...
- **`switch.<instance>_guider` reads off once guiding has been stopped**, even
  ...
  read on for the whole wait. That `LostLock` now reads off; one while guiding,
  or while guiding restarts after the wait, still reads on.
  `sensor.<instance>_guider_status` still reports N.I.N.A.'s state as it is.
