...
| — | nothing about `/equipment/focuser/last-af` | §4.4 scoped it to phase C — "phase C reads success from `/equipment/focuser/last-af` — R² against `RSquaredThreshold`" — and no task carried it. Added: the endpoint joins the tier table and the floor, `AUTOFOCUS-FINISHED` queues it, and `binary_sensor.autofocus_failed` gains its second arm. Without it the sensor catches only a HUNG run, and a run that finishes on a bad curve fit — the one that quietly costs a night of soft subs — raises nothing. |
